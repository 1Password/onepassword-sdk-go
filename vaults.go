...
type VaultsAPI interface {
	// List all vaults
	List(ctx context.Context) (*Iterator[VaultOverview], error)
}
...
// List all vaults
func (s VaultsSource) List(ctx context.Context) (*Iterator[VaultOverview], error) {
	resultString, err := clientInvoke(ctx, s.InnerClient, "VaultsList", map[string]interface{}{})
	if err != nil {
		return nil, err
	}
